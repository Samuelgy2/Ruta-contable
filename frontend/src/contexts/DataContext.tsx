...
interface DataContextType extends AppData {
  // Generic CRUD operations (for simplified components)
  create: (entity: keyof AppData, item: Record<string, unknown>) => void;
  update: (entity: keyof AppData, id: string, item: Record<string, unknown>) => void;
  remove: (entity: keyof AppData, id: string) => void;
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  // Generic CRUD operations (for simplified CRUD components)
  const create = (entity: keyof AppData, item: Record<string, unknown>) => {
    const newItem = { ...item, id: generateId() };
    setData(prev => {
      const entityData = prev[entity] as unknown[];
      return {
        ...prev,
        [entity]: [...entityData, newItem]
      };
    });
  };

  const update = (entity: keyof AppData, id: string, item: Record<string, unknown>) => {
    setData(prev => {
      const entityData = prev[entity] as { id: string }[];
      return {
        ...prev,
        [entity]: entityData.map((el) => 
          el.id === id ? { ...el, ...item } as typeof el : el
        )
      };
    });
  };

  const remove = (entity: keyof AppData, id: string) => {
    setData(prev => {
      const entityData = prev[entity] as { id: string }[];
      return {
        ...prev,
        [entity]: entityData.filter((el) => el.id !== id)
      };
    });
  };
