...
async function setupDatabase() {
  ...
  // ── periodos (UNA SOLA VEZ, sin comillas en nombres) ──────────────────────
  await pool.query(`
...
// Solo se activa si node-cron está instalado
try {
    ...
    try {
      ...
      await pool.query(
        `INSERT INTO periodos
           (anio, mes, nombreMes, fechaInicio, fechaFin, activo, cerrado, fechaCierre, observaciones, cerradoBy)
         VALUES ($1,$2,$3,$4,$5,false,true,NOW(),$6,$7)
         ON CONFLICT (anio, mes) DO UPDATE SET
           cerrado = true, fechaCierre = NOW()`,
        [
          yearNum, monthNum, monthName,
          `${yearNum}-${String(monthNum).padStart(2,'0')}-01`,
          `${yearNum}-${String(monthNum).padStart(2,'0')}-28`,
          'Cierre automático', adminName,
        ]
      );

      console.log(`✅ Cierre automático: ${monthName} ${yearNum}`);
    } catch (err) {
